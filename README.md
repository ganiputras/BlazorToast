...
```csharp
// Fire-and-forget (don't wait for user interaction)
ToastService.ShowInfo(builder => builder.AddContent(0, "Saved successfully"));

// Await simple close reason (enum)
var reason = await ToastService.ShowInfoAsync(builder => builder.AddContent(0, "Confirm action?"));
if (reason == ToastCloseReason.Click) { /* user clicked toast body */ }

// Await detailed result (includes timestamps and id)
...
- `ToastResultExampple.razor` — demonstrates `ToastResult` detailed output
